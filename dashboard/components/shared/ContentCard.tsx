...
import { Eye, Heart, Bookmark, CheckCircle, Clock, Users } from "lucide-react"
...
export function ContentCard({
  ...
  const isReel = media_type === "VIDEO" || media_type === "REEL"
  const aspectClass = isYT ? "aspect-video" : isReel ? "aspect-[9/16]" : "aspect-[4/5]"
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2.5 p-4">
        ...
        >
          <div className="flex items-center gap-1 text-[12px]" style={{ color: "var(--text-faint)" }} title="Visualizaciones (Plays)">
            <Eye size={11} />
            <span className="tabular-nums">{fmt(metrics.plays ?? metrics.reach)}</span>
          </div>
          <div className="flex items-center gap-1 text-[12px]" style={{ color: "var(--text-faint)" }} title="Alcance (Reach)">
            <Users size={11} />
