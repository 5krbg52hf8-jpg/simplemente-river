      ...
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            ...
            <div
              className="relative h-10 w-10 flex-shrink-0 overflow-hidden rounded-lg flex items-center justify-center bg-[var(--bg-elevated)]"
              style={{ border: "1px solid var(--border-subtle)" }}
            >
              {item.thumbnail ? (
                <Image src={item.thumbnail} alt={item.title} fill className="object-cover" sizes="40px" />
              ) : (
                <Instagram size={14} style={{ color: "var(--text-faint)" }} />
              )}
            </div>
