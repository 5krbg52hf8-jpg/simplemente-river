...
async function buildRAGContext(query: string): Promise<string> {
  ...
  const formatPostCompact = (p: typeof allPosts[0], rank?: string) => {
    ...
    const date = p.published_at
      ...
      : ''
    const metrics = m
      ? `Reach: ${fmt(m.reach)} | Likes: ${fmt(m.likes)} | Saves: ${fmt(m.saves)} | Shares: ${fmt(m.shares)} | ER: ${m.engagement_rate.toFixed(2)}%`
      : 'Sin métricas'
  ...
  const formatPost = (p: typeof allPosts[0], rank?: string) => {
    ...
    const date = p.published_at
      ...
      : ''
    const metrics = m
      ? `Reach: ${fmt(m.reach)} | Likes: ${fmt(m.likes)} | Saves: ${fmt(m.saves)} | Shares: ${fmt(m.shares)} | Comments: ${fmt(m.comments)} | ER: ${m.engagement_rate.toFixed(2)}%`
      : 'Sin métricas'
...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = `Sos el analista de contenido de Instagram de la cuenta @simplemente.river (una cuenta de River Plate). Tu trabajo es analizar datos reales de rendimiento de sus publicaciones para ayudarlo a entender qué funciona y cómo crear mejor contenido sobre River Plate.

<datos_disponibles>
Tenés acceso a: métricas por publicación (reach, likes, saves, shares, comments, engagement_rate expresado en %), captions, fechas de publicación, transcripciones de audio (campo "Transcripción" — solo para Reels), análisis IA (campo "Insights IA"), puntos de mejora y descripciones visuales de frames del video (campo "Visual").
Las publicaciones pueden ser de 3 tipos: Reels (VIDEO/REEL), Placas de imagen (IMAGE), o Carruseles (CAROUSEL_ALBUM). 
No todos los posts tienen transcripción — solo los Reels con audio pueden transcribirse. Si falta la transcripción, aclaralo, NO que "no tenés acceso" al dato.
