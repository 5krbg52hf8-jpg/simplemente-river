...
export function ChatInterface() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "¡Hola! Soy tu analista de contenido de **@simplemente.river**. Preguntame lo que quieras sobre tus publicaciones — reels, placas, carruseles, métricas, qué funcionó, qué mejorar.",
      ts: "ahora",
    },
  ])
  ...
  useEffect(() => {
    const isDefaultWelcomeOnly = messages.length === 1 && messages[0].id === "welcome" && history.length === 0
    if (!isDefaultWelcomeOnly) {
      try {
  ...
  function clearChat() {
    if (window.confirm("¿Estás seguro de que querés borrar el historial de chat?")) {
